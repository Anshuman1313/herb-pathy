...
const [activeGame, setActiveGame] = useState< Games | null>(null);
            ...
            <div className="header">
              ...
              <div className="header-inner">
                <div className="content-wrapper">
                  <motion.h2 layoutId={`h2-${activeGame.title}`} className="game-title ">{activeGame.title}</motion.h2>
                  <motion.p layoutId={`p-${activeGame.title}`} className="game-description">{activeGame.description}</motion.p>
                </div>
                <motion.button layoutId={`button-${activeGame.title}`} className="button">Get</motion.button>
              </div>
            </div>
            <motion.p 
               layout
      ...
      {GAMES.map((game) => (
        ...
        >
          ...
          <div className="game-wrapper">
            <div className="content-wrapper">
              <motion.h2 layoutId={`h2-${game.title}`} className="game-title">{game.title}</motion.h2>
              <motion.p layoutId={`p-${game.title}`} className="game-description">{game.description}</motion.p>
            </div>
            <motion.button layoutId={`button-${game.title}`} className="button">Get</motion.button>
          </div>
        </motion.li>
      ))}
    </ul>
