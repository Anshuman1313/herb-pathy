...
const PhotoGallery = () => {
  ...
  return (
       <div className="h-40 flex items-center relative w-[600px]  mx-auto">
      <MotionConfig transition={{ type: "spring", bounce: 0, duration: 4.4 }}>
        <motion.ul
          layout
          className={clsx(
            "mx-auto flex w-fit gap-2 ",
            activeBorder ? "border" : "border-0",
        ...
        <button
        onClick={()=>setActiveBorder((p)=>!p)}
        className={clsx(
            "relative cursor-pointer px-2 py-1 active:scale-[0.97] transition-all  rounded-lg text-sm outline-none  hover:bg-black/5",
            activeBorder ? "border border-green-500 border-dashed bg-black/5" : "border-0",
